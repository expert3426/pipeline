...
def pre_processing(datframe):
    ...
    df = datframe
    # #대문자 소문자로 변환
    df["rm_uppercase"] = df["tweet"].map(lambda x: x.lower())
    print(df.head())

    # 해쉬태그 또느 멘션 제거
    df["rm_hashtag"] = df["rm_uppercase"].str.replace(r'[\@\#]\S+', '')
    print(df.head())

    #HTML 제거
    df["rm_html"] = df["rm_hashtag"].str.replace(r'<\w+[^>]+>', '')
    print(df.head())

    #email 제거
    df["rm_email"] = df["rm_html"].str.replace(r'(\w+\.)*\w+@(\w+\.)+[a-z]+', '')
    print(df.head())

    # http, https 제거
    #www[\.][^ ]+ url 뒤 띄어쓰기 안 될 경우 있는지 확인 후
    df["rm_https"] = df["rm_email"].str.replace(r'(http|ftp|https)://[-\w.]+(:\d+)?(/([\w/_.]*)?)?|www[\.]\S+', '')
    print(df.head())

    # 마침표 제거
    df["rm_period"] = df["rm_https"].str.replace(r'[\.]', '')
    ...
    manual_sw_list = ['retweet', 'retwet', 'rt', 'oh', 'dm', 'mt', 'ht', 'ff', 'facebook', 'instagram', 'twitter',
                      'shoulda', 'woulda', 'coulda', 'might', "that's", "i'm", 'im', 'tb', 'mysql', 'hah']
    stop.extend(manual_sw_list)

    # print(stop)

    df['rm_stopword'] = df['rm_period'].apply(lambda x: ' '.join([word for word in x.split() if word not in (stop)]))
    print(df.head())

    # 표제어 추출. 동사 품사 기준으로 추출
    lem = WordNetLemmatizer()
    df['Normalization'] = df['rm_stopword'].apply(lambda x: ' '.join([lem.lemmatize(word, 'v') for word in x.split()]))
    print(df.head())

    # 구두점 제거 Normalization
    df["rm_puc"] = df["Normalization"].str.replace(r'[^\w\s]', '')
    print(df.head())

    # 불필요한 숫자 제거
    df["rm_number"] = df["rm_puc"].str.replace(r'[0-9]+', '')
    print(df.head())

    # 두 글자 이상 중복된 경우 알파벳 모두 두 글자로 만드는 작업
    df['rm_duplicate'] = df['rm_number'].str.replace(r'([a-z])\1{1,}', r'\1')

    print(df.head())
    # 인코딩으로 인한 깨진 문자 제거
    df['rm_non_alphabet'] = df['rm_duplicate'].apply(
        lambda x: ' '.join([word for word in x.split() if non_alphabet.search(word) is None]))
    print(df.head())

    #haha 여러번 중복 되는 것 하나로 변경
    df['unique_haha'] = df['rm_non_alphabet'].str.replace(r'(ha)\1{1,}', r'\1')
    print(df.head())
...
class Model():
    ...
    df = df.drop(
        ["rm_uppercase", "rm_hashtag", "rm_html", "rm_email", "rm_https", "rm_period", "rm_stopword", "Normalization",
         "rm_puc", "rm_number", "rm_non_alphabet", "unique_haha"], axis=1)
